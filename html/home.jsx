          ...
          <tbody>
          {
            this.state.posts.map(function (post, index) {
              return <tr key={index}>
                ...
                <td>{post.subject}</td>
                <td>
                  <button onClick={() => self.updatePost(post._id)} className="btn btn-link">
                    <span className="glyphicon glyphicon-pencil"/>
                  </button>
                </td>
                <td>
                  <button onClick={() => self.deletePost(post._id)} className="btn btn-link">
  ...
  getPostWithId() {
    ...
    axios.get('/posts/' + id)
        .then(function (response) {
          ...
          if (response) {
            self.setState({title: response.data.title, subject: response.data.subject, id: response.data._id});
          }
        })
          ...
          <div className="form-area">
            <form role="form">
              ...
              <button type="button"
                      ...
                      onClick={hasId ? this.updatePost : this.addPost}>{hasId ? 'Update' : 'Add'} Post
              </button>
            </form>
          </div>
        </div>
